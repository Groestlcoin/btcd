...
func (c *Client) BackendVersion() (BackendVersion, error) {
	...
	case nil:
		log.Debugf("Detected grsd version: %v", info.Version)
		version := Btcd
	...
	case *btcjson.RPCError:
		if err.Code != btcjson.ErrRPCMethodNotFound.Code {
			return 0, fmt.Errorf("unable to detect grsd version: "+
				"%v", err)
		}

	default:
		return 0, fmt.Errorf("unable to detect grsd version: %v", err)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("unable to detect groestlcoind version: %v", err)
	}

	// Parse the bitcoind version and cache it.
	log.Debugf("Detected groestlcoind version: %v", networkInfo.SubVersion)
