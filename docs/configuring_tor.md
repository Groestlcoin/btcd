...
grsd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, grsd supports a hybrid,
...
While it is easier to only run as a client, it is more beneficial to the Groestlcoin
...
Configuring grsd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the grsd command
line or in the grsd configuration file.  Typically the Tor proxy address will be
...
By default, grsd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since grsd will act as the server.
...
```bash
$ ./grsd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
grsd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the grsd command line or in the grsd
...
```bash
$ ./grsd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
grsd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./grsd --proxy=127.0.0.1:9050 --torisolation
```
