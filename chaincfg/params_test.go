...
func TestSigNetPowLimit(t *testing.T) {
	...
	// Workaround bug in Groestlcoin Signet genesis block. PowLimit in genesis
	// block does not match actual PowLimit.
	sigNetPowLimitGenesisHex, _ := hex.DecodeString(
		"000000ffff000000000000000000000000000000000000000000000000000000",
	)
	powLimitGenesis := new(big.Int).SetBytes(sigNetPowLimitGenesisHex)
	if compactToBig(sigNetGenesisBlock.Header.Bits).Cmp(powLimitGenesis) != 0 {
		t.Fatalf("Signet PoW limit header bits (%x) not equal to big "+
